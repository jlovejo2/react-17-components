...
export default async function handler(req, res) {
    ...
    async function postMethod() {
        try {
            ...
            console.log(`POST api/speakers/${id} error: `, err);
            res.status(500).send(`POST api/speakers/${id} status 500 unexepected error`);
        }       
    }
    ...
    async function putMethod() {
        try {
            ...
            console.log(`PUT api/speakers/${id} error: `, err);
            res.status(500).send(`PUT api/speakers/${id} status 500 unexepected error`);
        }       
    }
    ...
    async function deleteMethod() {
        try {
            ...
            if (!speakers) {
                ...
                console.log('delete method ...', speakers)
                const newSpeakerArray = speakers.filter(function (rec) {
                    return rec.id != id;
                });
                writeFile(jsonFile, JSON.stringify({speakers: newSpeakerArray}, null, 2))
                res.setHeader("Content-Type", "application/json");
                res.status(200).send(JSON.stringify(speakers.find(rec=>rec.id), null, 2));
                console.log(`DELETE /api/speakers/${id} status: 200`);
            }
        } catch (err){
            console.log(`DELETE api/speakers/${id} error: `, err);
            res.status(500).send(`DELETE api/speakers/${id} status 500 unexepected error`);
        }       
    }
